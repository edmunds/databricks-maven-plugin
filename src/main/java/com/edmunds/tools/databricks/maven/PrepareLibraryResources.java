...
/**
 * Prepares the library-mapping.json file such that we can run library attachment,
 * scans project later (e.g. during a build).
 */
...
public class PrepareLibraryResources extends BaseLibraryMojo {
    ...
    @Parameter(property = "libaryMappingFile", defaultValue = "${project.build.directory}/databricks-plugin/"
        + LIBRARY_MAPPING_FILE_NAME)
    protected File libraryMappingFileOutput;
    ...
    void prepareLibraryResources() throws MojoExecutionException {
        if (project.getArtifact().getType().equals(JAR)) {
            if (ArrayUtils.isNotEmpty(clusters)) {
                try {
                    FileUtils.writeStringToFile(libraryMappingFileOutput,
