...
public class UpsertClusterMojo extends BaseDatabricksMojo {
    ...
    private File dbClusterFile;

    /**
     * If true, command will fail if cluster with specified name already exists.
     */
    @Parameter(property = "failOnClusterExists")
    private boolean failOnClusterExists = true;

    public void execute() throws MojoExecutionException {
        ClusterTemplateModel ct;
        try {
            ct = OBJECT_MAPPER.readValue(dbClusterFile, ClusterTemplateModel.class);
        } catch (IOException e) {
        ...
        try {
            ...
            libraryService.install(clusterId, getLibraryDTO(ct.getArtifactPaths()));
    ...
    private LibraryDTO[] getLibraryDTO(Collection<String> artifactPaths) {
        int libsCount = artifactPaths.size();
        LibraryDTO[] libs = new LibraryDTO[libsCount];
        int i = 0;
